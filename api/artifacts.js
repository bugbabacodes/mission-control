...
// Get any artifacts saved by quick-action API (shared global)
function getQuickActionArtifacts() {
  return global.quickActionArtifacts || [];
}
...
const SAMPLE_ARTIFACTS = [
  {
    ...
    agentId: 'shakespeare',
    type: 'linkedin_post',
    title: 'Why Most Startups Fail at Zero to One',
    summary: 'Post about execution vs planning',
    content: `I've shipped 100+ projects. Here's the pattern I see in every failure:

// The Trap

Founders spend 6 months "getting ready":
- Perfect pitch decks
- Endless market research
- Waiting for the "right time"

Meanwhile, their competitors ship ugly v1s and learn.

// What Actually Works

1. Set a ship date BEFORE you start building
2. Work backward from there
3. Cut features ruthlessly
4. Ship something embarrassing

// The Truth

The best founders I know? They're not the smartest. They're the fastest.

Ideas are cheap. Execution is everything.

What's something you've been "preparing" for too long? 👇`,
...
module.exports = async (req, res) => {
  ...
  // GET - List artifacts
  if (req.method === 'GET') {
    // Merge sample artifacts with quick action artifacts
    const quickArtifacts = getQuickActionArtifacts();
    let all = [...quickArtifacts, ...artifacts];
    
    if (agentId) {
      all = all.filter(a => a.agentId === agentId);
    }
